...
# -*- coding: utf-8 -*-
"""
Train an XGBoost NEI-6 model (2015–2019) using PM features only
...
def main():
    print('🎯 XGBoost Enhanced NEI-6 (2015–2019) — PM-only + ΔSBP≥20, NO EMS severity composites')
    ...
    out_dir = os.path.join(base_dir, f'xgb_enhanced_pm_only_sbpDelta_noSeverity_2015to2019_{ts}')
    ...
    try:
        ...
        if imp is not None and len(imp) == len(feature_names):
            wrap_df = pd.DataFrame({'feature': feature_names, 'importance': imp}).sort_values('importance', ascending=False)
            wrap_df.to_csv(os.path.join(out_dir, 'feature_importance_xgb_wrapper.csv'), index=False)
            top_k = 30
            top_df = wrap_df.head(top_k).iloc[::-1]
            plt.figure(figsize=(10, max(6, len(top_df) * 0.35)))
            sns.barplot(x='importance', y='feature', data=top_df, color='#DD8452')
            plt.title('XGBoost Feature Importance (wrapper, top 30) — PM-only + ΔSBP≥20')
            plt.xlabel('Importance')
            plt.ylabel('Feature')
            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, 'feature_importance_wrapper_top30.png'), dpi=300, bbox_inches='tight')
            plt.close()
    ...
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate'); plt.title('XGB PM-only + ΔSBP≥20 (2015–2019) — no severity composites')
