...
# -*- coding: utf-8 -*-
"""
Train a HistGradientBoostingClassifier NEI-6 model (2015–2019) using PM features only
...
def build_enhanced_nei6_label(df: pd.DataFrame, df_proc: pd.DataFrame, year: int, id_col_proc: str) -> pd.Series:
    ...
    packed_rbc_5units = (pd.to_numeric(df[rbc_col], errors='coerce') >= 5.0) if rbc_col in df.columns else pd.Series(False, index=df.index)
    angio = (df['ANGIOGRAPHY'] == 1) if 'ANGIOGRAPHY' in df.columns else pd.Series(False, index=df.index)
    if 'ANGIOGRAPHYHRS' in df.columns:
        angio = angio | (pd.to_numeric(df['ANGIOGRAPHYHRS'], errors='coerce') <= 24).fillna(False)
    any_operation = (pd.to_numeric(df['HMRRHGCTRLSURGTYPE'], errors='coerce') > 0) if 'HMRRHGCTRLSURGTYPE' in df.columns else pd.Series(False, index=df.index)
...
def main():
    print('🎯 HistGradientBoosting Enhanced NEI-6 (2015–2019) — PM-only + ΔSBP≥20, NO EMS severity composites')
    ...
    t0 = time.time(); print('⏳ Fitting HistGradientBoosting ...')
    ...
    print(f"🏆 HGB AUC={auc:.4f} (fit {fit_sec:.1f}s)")

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_dir = os.path.join(base_dir, f'hgb_enhanced_pm_only_sbpDelta_noSeverity_2015to2019_{ts}')
    ...
    # Permutation-based feature importance for HGB
    try:
        ...
        fi_df.to_csv(os.path.join(out_dir, 'feature_importance_hgb_permutation.csv'), index=False)

        top_k = 30
        top_df = fi_df.head(top_k).iloc[::-1]
        plt.figure(figsize=(10, max(6, len(top_df) * 0.35)))
        sns.barplot(x='importance_mean', y='feature', data=top_df, color='#C44E52')
        plt.title('HGB Permutation Importance (AUC, top 30) — PM-only + ΔSBP≥20')
        plt.xlabel('Mean importance (AUC drop)')
        plt.ylabel('Feature')
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, 'feature_importance_permutation_top30.png'), dpi=300, bbox_inches='tight')
        plt.close()
    except Exception as e:
        print(f"⚠️ Failed to compute HGB permutation importance: {e}")

    plt.figure(figsize=(10, 8))
    plt.plot(fpr, tpr, label=f"HGB AUC={auc:.3f}")
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate'); plt.title('HGB PM-only + ΔSBP≥20 (2015–2019) — no severity composites')
