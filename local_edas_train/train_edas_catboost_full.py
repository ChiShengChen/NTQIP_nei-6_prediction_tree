...
# -*- coding: utf-8 -*-
"""
Train a CatBoost model on the EDAS dataset from a single CSV.
...
def main():
    parser = argparse.ArgumentParser(description='Train CatBoost on EDAS CSV (PM-only style features).')
    ...
    out_dir = os.path.join(base_dir, f'catboost_edas_pm_only_sbpDelta_{ts}')
    ...
    try:
        ...
        if importances is not None and len(importances) == len(feature_names):
            ...
            sns.barplot(x='importance', y='feature', data=top_df, color='#00A8CC')
            plt.title('CatBoost Feature Importance (top 30) — EDAS')
