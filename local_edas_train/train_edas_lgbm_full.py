...
# -*- coding: utf-8 -*-
"""
Train a LightGBM model on the EDAS dataset from a single CSV.
...
def main():
    parser = argparse.ArgumentParser(description='Train LightGBM on EDAS CSV (PM-only style features).')
    ...
    print(f"🏆 LightGBM AUC={auc:.4f} (fit {fit_sec:.1f}s)")

    # Outputs
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_dir = os.path.join(base_dir, f'lgbm_edas_pm_only_sbpDelta_{ts}')
    ...
    try:
        ...
        if importances is not None and len(importances) == len(feature_names):
            ...
            fi_df.to_csv(os.path.join(out_dir, 'feature_importance_lgbm.csv'), index=False)
            top_k = 30
            top_df = fi_df.head(top_k).iloc[::-1]
            plt.figure(figsize=(10, max(6, len(top_df) * 0.35)))
            sns.barplot(x='importance', y='feature', data=top_df, color='#7FB069')
            plt.title('LightGBM Feature Importance (top 30) — EDAS')
