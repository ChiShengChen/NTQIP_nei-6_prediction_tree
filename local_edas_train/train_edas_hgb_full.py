...
# -*- coding: utf-8 -*-
"""
Train a Histogram Gradient Boosting model on the EDAS dataset from a single CSV.
...
def main():
    parser = argparse.ArgumentParser(description='Train HistGradientBoosting on EDAS CSV (PM-only style features).')
    ...
    print('🎯 EDAS HistGradientBoosting')
    ...
    # Calculate class_weight for class imbalance
    ...
    out_dir = os.path.join(base_dir, f'hgb_edas_pm_only_sbpDelta_{ts}')
    ...
    # Feature names (used for both importance calculations)
    ...
    try:
        ...
        if importances is not None and len(importances) == len(feature_names):
            ...
            fi_df.to_csv(os.path.join(out_dir, 'feature_importance_hgb.csv'), index=False)
            top_k = 30
            top_df = fi_df.head(top_k).iloc[::-1]
            plt.figure(figsize=(10, max(6, len(top_df) * 0.35)))
            sns.barplot(x='importance', y='feature', data=top_df, color='#F18F01')
            plt.title('HistGradientBoosting Feature Importance (top 30) — EDAS')
        ...
        print(f"⚠️ Failed to compute HGB feature importance: {e}")
    ...
    try:
        ...
        perm_df.to_csv(os.path.join(out_dir, 'permutation_importance_hgb.csv'), index=False)
        
        # Plot top 30 permutation importance
        top_k = 30
        top_perm_df = perm_df.head(top_k).iloc[::-1]
        plt.figure(figsize=(10, max(6, len(top_perm_df) * 0.35)))
        plt.barh(range(len(top_perm_df)), top_perm_df['importance_mean'], color='#F18F01')
        plt.errorbar(
            top_perm_df['importance_mean'], 
            range(len(top_perm_df)),
            xerr=top_perm_df['importance_std'],
            fmt='none',
            color='black',
            capsize=3
        )
        plt.yticks(range(len(top_perm_df)), top_perm_df['feature'])
        plt.xlabel('Permutation Importance (AUC decrease)')
        plt.ylabel('Feature')
        plt.title('HistGradientBoosting Permutation Importance (top 30) — EDAS')
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, 'permutation_importance_top30.png'), dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Permutation importance computed and saved")
    except Exception as e:
        print(f"⚠️ Failed to compute permutation importance: {e}")
