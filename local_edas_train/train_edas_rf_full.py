...
# -*- coding: utf-8 -*-
"""
Train a RandomForest model on the EDAS dataset from a single CSV.
...
def main():
    parser = argparse.ArgumentParser(description='Train RandomForest on EDAS CSV (PM-only style features).')
    ...
    out_dir = os.path.join(base_dir, f'rf_edas_pm_only_sbpDelta_{ts}')
    ...
    try:
        ...
        if importances is not None and len(importances) == len(feature_names):
            ...
            plt.title('RandomForest Feature Importance (top 30) — EDAS')
