...
# -*- coding: utf-8 -*-
"""
Train a LightGBM NEI-6 model (2015–2019) using PM features only
...
def main():
    print('🎯 LightGBM Enhanced NEI-6 (2015–2019) — PM-only + ΔSBP≥20, NO EMS severity composites')
    ...
    t0 = time.time(); print('⏳ Fitting LightGBM ...')
    ...
    print(f"🏆 LGBM AUC={auc:.4f} (fit {fit_sec:.1f}s)")

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_dir = os.path.join(base_dir, f'lgbm_enhanced_pm_only_sbpDelta_noSeverity_2015to2019_{ts}')
    ...
    try:
        ...
        gain_df = pd.DataFrame({'feature': feature_names, 'importance_gain': fi_gain}).sort_values('importance_gain', ascending=False)
        gain_df.to_csv(os.path.join(out_dir, 'feature_importance_lgbm_gain.csv'), index=False)
        top_k = 30
        top_gain = gain_df.head(top_k).iloc[::-1]
        plt.figure(figsize=(10, max(6, len(top_gain) * 0.35)))
        sns.barplot(x='importance_gain', y='feature', data=top_gain, color='#55A868')
        plt.title('LightGBM Feature Importance (gain, top 30) — PM-only + ΔSBP≥20')
        plt.xlabel('Gain')
        plt.ylabel('Feature')
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, 'feature_importance_gain_top30.png'), dpi=300, bbox_inches='tight')
        plt.close()
    except Exception as e:
        print(f"⚠️ Failed to compute LightGBM feature importance: {e}")

    plt.figure(figsize=(10, 8))
    plt.plot(fpr, tpr, label=f"LGBM AUC={auc:.3f}")
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate'); plt.title('LGBM PM-only + ΔSBP≥20 (2015–2019) — no severity composites')
