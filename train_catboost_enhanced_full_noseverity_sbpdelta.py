...
# -*- coding: utf-8 -*-
"""
Train a CatBoost NEI-6 model (2015–2019) using PM features only
...
def main():
    print('🎯 CatBoost Enhanced NEI-6 (2015–2019) — PM-only + ΔSBP≥20, NO EMS severity composites')
    ...
    t0 = time.time(); print('⏳ Fitting CatBoost ...')
    ...
    print(f"🏆 CatBoost AUC={auc:.4f} (fit {fit_sec:.1f}s)")

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_dir = os.path.join(base_dir, f'catboost_enhanced_pm_only_sbpDelta_noSeverity_2015to2019_{ts}')
    ...
    try:
        ...
        fi_df.to_csv(os.path.join(out_dir, 'feature_importance_catboost.csv'), index=False)
        top_k = 30
        top_df = fi_df.head(top_k).iloc[::-1]
        plt.figure(figsize=(10, max(6, len(top_df) * 0.35)))
        sns.barplot(x='importance', y='feature', data=top_df, color='#4C72B0')
        plt.title('CatBoost Feature Importance (top 30) — PM-only + ΔSBP≥20')
        ...
        print(f"⚠️ Failed to compute CatBoost feature importance: {e}")

    plt.figure(figsize=(10, 8))
    plt.plot(fpr, tpr, label=f"CatBoost AUC={auc:.3f}")
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate'); plt.title('CatBoost PM-only + ΔSBP≥20 (2015–2019) — no severity composites')
